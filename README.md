...
- **Bring-your-own-key fallback** — switch to OpenAI, OpenRouter, or any
  OpenAI-compatible `/chat/completions` endpoint (Groq, Together, vLLM,
  LM Studio, Ollama, Gemini's OpenAI shim, etc.). Key is stored only in
  `localStorage`.
...
| ------------ | ------------------------------------------------- | -------------------------------------------------- |
| **Puter**    | `claude-sonnet-4-5`, `gpt-5.4-nano`, `gemini-2.5-flash` | Keyless, user-pays via Puter account.        |
| OpenAI       | `gpt-4o-mini` or `gpt-4o`                         | Bring your own key.                                |
| OpenRouter   | `anthropic/claude-3.5-sonnet`                     | One key, hundreds of models incl. free-tier ones.  |
| Custom       | Any OpenAI-compatible model                       | Groq, Together, vLLM, LM Studio, Ollama, Gemini's OpenAI shim, etc. |
